...
var seasons = [
    ...
    new Season(
        ...
        [
            ...
            new Team(ScoringSystem.twoone, 2021, "AK", 7, 7, [72.9, 79.16, 95.62, 79.32, 151.4, 142.12, 84.9, 112.66, 72.66, 133.64, 101.56, 91, 79, 144.92]),
            new Team(ScoringSystem.twoone, 2021, "Kev & Eazy", 6, 8, [103.36, 99.12, 129.58, 76.26, 121.44, 96.18, 84.44, 129.9, 86.08, 65, 88.08, 95.44, 108.82, 111.42]),
            new Team(ScoringSystem.twoone, 2021, "Palko", 10, 4, [60.46, 134, 97.94, 119.22, 142.96, 107.3, 115.46, 88.06, 82.88, 77.68, 111, 121.34, 95.62, 125.94]),
            new Team(ScoringSystem.twoone, 2021, "Tipples", 8, 6, [116.98, 114.42, 100.6, 119.92, 136.18, 98.38, 105.14, 65, 59.64, 105.4, 92.3, 74.52, 133.46, 118.22]),
            new Team(ScoringSystem.twoone, 2021, "Cruz", 5, 9, [122.24, 61.62, 80.42, 95, 118.8, 95.94, 119.76, 126, 65.36, 70.92, 71.02, 106.88, 65.4, 104.58]),
            new Team(ScoringSystem.twoone, 2021, "Tdub", 8, 6, [114.52, 72.48, 120.02, 89.22, 104.78, 97.9, 96.52, 87.36, 86.48, 95.34, 76.64, 49.2, 95.02, 87.94]),
            new Team(ScoringSystem.twoone, 2021, "Spolt", 4, 10, [89.56, 94, 56.54, 76.64, 76.92, 91.56, 96.74, 71.82, 84.56, 103.5, 100.88, 76.02, 111.78, 96.2]),
            new Team(ScoringSystem.twoone, 2021, "BJ", 5, 9, [112.56, 105.62, 83.42, 68.66, 65.42, 53.7, 80.54, 94.92, 88.18, 75.12, 90.68, 83.76, 81.7, 94.04]),
            new Team(ScoringSystem.twoone, 2021, "Ram", 8, 6, [103.36, 107, 105.34, 100.52, 117.26, 106.56, 95.84, 46.36, 84.2, 53.78, 90.72, 93.22, 77.56, 99.9]),
        ]
    ),
    new Season(
        ScoringSystem.twoone,
...
class Standings extends React.Component {
    ...
    constructor(props){
        ...
        fetch('https://docs.google.com/spreadsheets/d/1sLyvQ862hHkdiEzd3p9XlDHrcsXONALO-7AEDjo83tw/gviz/tq?&sheet=2022&tq=' + query)
            .then(res => res.text())
            .then(rep => {
        ...
        fetch('https://docs.google.com/spreadsheets/d/1sLyvQ862hHkdiEzd3p9XlDHrcsXONALO-7AEDjo83tw/gviz/tq?&sheet=2023&tq=' + query)
            .then(res => res.text())
            .then(rep => {
                    var season = new Season(ScoringSystem.twoone, 2023, []);
                    var data = JSON.parse(rep.substring(47).slice(0, -2));
                    data.table.rows.forEach(team => {
                        var scores = _.map(team.c.slice(2), function (score, index){
                            return score.v;
                        });
                        season.teams.push(new Team(ScoringSystem.twoone, 2023, team.c[0].v, team.c[1].v, 0, scores));
                    });
        ...
        fetch('https://docs.google.com/spreadsheets/d/1sLyvQ862hHkdiEzd3p9XlDHrcsXONALO-7AEDjo83tw/gviz/tq?&sheet=2025&tq=' + query)
            .then(res => res.text())
            .then(rep => {
                    var season = new Season(ScoringSystem.twoone, 2025, []);
                    var data = JSON.parse(rep.substring(47).slice(0, -2));
                    data.table.rows.forEach(team => {
                        var scores = _.map(team.c.slice(2), function (score, index){
                            return score.v;
                        });
                        season.teams.push(new Team(ScoringSystem.twoone, 2025, team.c[0].v, team.c[1].v, 0, scores));
                    });
        ...
        fetch('https://docs.google.com/spreadsheets/d/1sLyvQ862hHkdiEzd3p9XlDHrcsXONALO-7AEDjo83tw/gviz/tq?&sheet=2026&tq=' + query)
            .then(res => res.text())
            .then(rep => {
                    var season = new Season(ScoringSystem.twoone, 2026, []);
                    var data = JSON.parse(rep.substring(47).slice(0, -2));
                    data.table.rows.forEach(team => {
                        var scores = _.map(team.c.slice(2), function (score, index){
                            return score.v;
                        });
                        season.teams.push(new Team(ScoringSystem.twoone, 2026, team.c[0].v, team.c[1].v, 0, scores));
                    });
